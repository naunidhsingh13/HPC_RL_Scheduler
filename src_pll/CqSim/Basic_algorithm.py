...
class Basic_algorithm:
    ...
    def make_feature_vector(self, jobs, system_status, job_cols, window_size):
        ...
        fv = np.zeros((1, input_dim[0], input_dim[1]), dtype=np.float32)  # ?
        # set X locations
    ...
    def compute_score_probabilities(self, wait_job, node_struc, currentTime, para_list=None, epsilon=0.25,
                                    backfill=False):
        # self.debug.debug("* "+self.myInfo+" -- get_score",5)
        # self.scoreList = []
        waitNum = len(wait_job)
